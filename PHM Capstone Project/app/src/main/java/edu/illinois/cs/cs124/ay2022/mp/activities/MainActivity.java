...
public final class MainActivity extends AppCompatActivity
  ...
  private void updateShownPlaces(final List<Place> showPlaces) {
    ...
    for (Place place : showPlaces) {
      ...
      Double lat = (place.getLatitude() - 40.10986682167534) * 69;
      Double lon = (place.getLongitude() - -88.22831928981661) * 54.6;
      DecimalFormat df1 = new DecimalFormat("#.#");
      DecimalFormat df2 = new DecimalFormat("#");
      String distance = "";
      String time = "";
      if (df1.format(Math.sqrt((lat * lat) + (lon * lon))).equals("0")) {
        distance = "You are here!";
      } else {
        distance = df1.format(Math.sqrt((lat * lat) + (lon * lon))) + " miles, ";
        time = df2.format(Math.sqrt((lat * lat) + (lon * lon)) * 17) + " minute walk";
      }

      //If the favorite button is pressed, add a "Favorited!" to the description
      if (getIntent().getStringExtra("favorite") == null) {
        marker.setTitle(place.getDescription() + "\n" + "\n" + distance + time);
      } else {
        marker.setTitle(place.getDescription() + "\n" + "\n" + distance + ", " + time
            + "\n" + "\n" + getIntent().getStringExtra("favorite"));
      }
  ...
  // Change the markers on the map to match the text the user inputs
  @Override
  public boolean onQueryTextChange(final String text) {
    // Create a list of places that match the text from the whole list of places
    List<Place> y = search(allPlaces, text);
    // If there are no matches, show all the places. Otherwise, show the places that match
    if (y.size() == 0) {
      updateShownPlaces(allPlaces);
    } else {
      updateShownPlaces(y);
  ...
  public boolean longPressHelper(final GeoPoint p) {
    ...
    // Added latitude and longitude values to the app as extras
